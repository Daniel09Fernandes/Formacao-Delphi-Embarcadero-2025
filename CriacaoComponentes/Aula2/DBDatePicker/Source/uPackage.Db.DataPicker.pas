...
begin
  inherited;
  if not((csReading in ComponentState) or (csLoading in ComponentState)) and
    (not FOnChanged) then
    FOnChanged := False;
end;
...
begin
  if (IsValidDataLink) and ((FDataLink.Field.IsNull) or
    VarIsEmpty(FDataLink.Field.Value)) then
  begin
    Date := Now;
  end
  else
